...
#define WIFI_SSID "pozdrawiam"
#define WIFI_PASSWORD "osiekrulz"
...
#define AUTHOR_EMAIL "osiek.arduino@gmail.com"
#define AUTHOR_PASSWORD "nrfn koak cquj fmii"

/* Recipient's email*/
#define RECIPIENT_EMAIL "oosiekk@gmail.com"
...
float PROGRAM_VERSION = 10.1;
//10.1 - mailing, fix zegara
// 9.16 - chyba naprawione porty
// 9.x - sieciowe
// 8.x - fix obsługi klienta, optymalizacja
...
void setup() {
  ...
  */
  ...
  if(WiFi.localIP()[3] == 36){
      ...
      Serial.println(WiFi.localIP());
      Serial.println("Niemienione ssidy");
  }
  ...
  */
  ...
  message.subject = nazwaHosta;
  message.addRecipient(F("Osiek"), RECIPIENT_EMAIL);
  /*Send HTML message*/
  ...
  String textMsg = "Uruchomiłem się "+IpAddress2String(WiFi.localIP());
...
String emailMessage = "";
bool statusObslugiKlienta=0;
int czekanieObslugiKlienta=10;
...
void loop() {
  ...
  */
  ...
  if (currentMillis - previousMillis >= ms) {
    ...
    statusObslugiKlienta=ObslugaKlienta();
    if(statusObslugiKlienta){
      czekanieObslugiKlienta=6000;
    }else if(czekanieObslugiKlienta>0){
      czekanieObslugiKlienta=czekanieObslugiKlienta-1;
    }
  ...
  if(emailMessage != "" && czekanieObslugiKlienta==0){
    emailMessage=currentDate+" - "+emailMessage;
    /****************************LOOP MAILING***********************/
    MailClient.networkReconnect(true);
    /** Enable the debug via Serial port
      * 0 for no debugging
...
bool ObslugaKlienta(){
    ...
    if (client) {
      ...
      client.println("<hr>");
      if(WiFi.localIP()[3]==14){
        client.println("<p class='w3-large'><b><i class='fa fa-asterisk fa-fw w3-margin-right w3-text-teal'></i>Pompa jest");
        if(going_up==1){
        ...
        client.println("</p>");
        client.println("<span style='font-size:100px;'><a href=\"/WIFI_CONTROL=UP\">&#9745;</a></span>");
